...
MainWindow::MainWindow(int screen, QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow()){
    ...
    connect(ui->combo_desk_layout, SIGNAL(currentIndexChanged(int)), this, SLOT(desktimechanged()) );
    ...
    // Load settings for the specified screen
    LoadSettings();
}
...
void MainWindow::SaveSettings(){
    ...
    ui->statusbar->showMessage(tr("Settings saved successfully"), 3000);
}
...
void MainWindow::LoadSettings(){
    ...
    for(int i=0; i<bgs.length(); i++){
        if(bgs[i]=="default"){ 
            ui->combo_desk_bg->addItem( QIcon(DEFAULTBG), tr("System Default"), bgs[i] ); 
        }
        else if(bgs[i].startsWith("rgb(")){
            ui->combo_desk_bg->addItem(QString(tr("Solid Color: %1")).arg(bgs[i]), bgs[i]); 
        }
        else{ 
    ...
    loading = false;
    deskbgchanged(); // Update the preview
}
...
//        PRIVATE
//=================
QString MainWindow::getColorStyle(QString current, bool allowTransparency){
    QString out;
    ...
    if(allowTransparency){ 
        ncol= QColorDialog::getColor(ccol, this, tr("Select Color"), QColorDialog::ShowAlphaChannel); 
    }
    else{ 
        ncol= QColorDialog::getColor(ccol, this, tr("Select Color")); 
    }
...
void MainWindow::deskbgchanged(){
    ...
    //Keep track of current background for change detection
    if(!loading && ui->radio_desk_single->isChecked() && cBG!=ui->combo_desk_bg->currentIndex()){ 
        // Settings changed - could auto-save here or show save button
    }
    cBG = ui->combo_desk_bg->currentIndex();
...
void MainWindow::desktimechanged(){
    if(!loading){ 
        // Settings changed - could auto-save here
    }
}

void MainWindow::deskbgremoved(){
    if(ui->combo_desk_bg->count()<1){ return; } //nothing to remove
    ui->combo_desk_bg->removeItem( ui->combo_desk_bg->currentIndex() );
}

void MainWindow::deskbgadded(){
...
void MainWindow::deskbgcoloradded(){
...
void MainWindow::deskbgdiradded(){
...
void MainWindow::deskbgdirradded(){
...
void MainWindow::showAbout(){
    QMessageBox::about(this, tr("About Wallpaper Settings"), 
        tr("LCC Wallpaper Settings\n\n"
           "Configure desktop wallpaper and background settings for the Lumina Desktop Environment.\n\n"
           "Part of the Lumina Control Center (LCC) suite of applications.\n\n"
           "Copyright (c) 2024, jt(q5sys)\n"
           "Available under the 3-clause BSD license"));
}
